<template>
  <div>
    ...
    <el-card class="user_list_card">
      ...
      <span>注释：删除上级病种时会将该病种下所有病种删除！</span>
      <el-table
          :data="currentDisease"
...
export default {
  ...
  methods: {
    ...
    selectDisease(){
      ...
      for(var i=0;i<this.tableData.length;i++){
        for(var j=0;j<this.tableData[i].children.length;j++){
          if(this.tableData[i].children[j].label === this.selected){
            this.currentDisease.push(this.tableData[i].children[j])
          }
        }
    ...
    addDisease(){
      getCategory("/api/category/checkDiseaseName/"+this.addDiseaseParam.firstDisease).then((response)=> {
        if(response.code==200){
          this.addDiseaseParam.username = sessionStorage.getItem("user");
          this.addDiseaseParam.uid = sessionStorage.getItem("uid");
          //如果为空则添加的是一级病种
          if(this.addDiseaseParam.parentId == ""){
            this.addDiseaseParam.parentId = "1";
          }
          console.log("addDiseaseParam")
    ...
    getInfoDisease(row){
      ...
      this.updateDiseaseParam.username = sessionStorage.getItem("user");
      this.updateDiseaseParam.uid = sessionStorage.getItem("uid");
    ...
    getId(data){
      ...
      console.log("👉"+this.updateDiseaseParam.parentId)
    },
    updateDisease(){
      // getCategory("/api/category/checkDiseaseName/"+this.updateDiseaseParam.diseaseName).then((response)=> {
      //       if(response.code==200){
      //
      //       }else{
      //         this.$message.error("请填写有效的病种名称")
    ...
    deleteDisease(row) {
      ...
      if(row.tableNum0==0&&row.tableNum1==0&&row.tableNum2==0){
        ...
        this.deleteDis.username = sessionStorage.getItem("user");
        this.deleteDis.uid = sessionStorage.getItem("uid");
        const data = {
          list:this.deleteIds
        }
        console.log(data)
