...
export default {
  ...
  methods: {
    ...
    toXLSX() {
      // 发送请求给后端，传递表名和选定的字段
      getRequest(`/TableData/getInfoByTableName/${this.showDataForm.tableName}`).then(
          (res) => {
            //二维数组 包含表头以及每一行数据
            const final_data = [];
    ...
    getTableData(tableId, tableName) {
      getTableData("/TableData/getTableData", tableId, tableName)
          .then((response) => {
            // 获取表数据
    ...
    getData(tablename) {
      ...
      getRequest("/TableData/DataTable/getInfoByTableName", {
        tableName: tablename,
      }).then((res) => {
