<template>
  <div>
    ...
    <el-divider/>
    <!--           基本信息                         -->
    <div class="mainPage" v-if="showView === 'baseInfo'" v-loading="dataList_loading" element-loading-text="正在分析数据">
      <el-form ref="modeInfo" :rules="modeRules" :model="modeInfo" label-width="90px">
        <el-form-item label="训练名称:" prop="modelname">
          <i>输入一个名称，可包含中文、英文、数字和常见特殊字符，最多 30 个字符</i>
          <el-input v-model="modeInfo.modelname"></el-input>
        </el-form-item>
        <el-form-item label="备注:" prop="remarks">
          <i>输入执行备注，用于备忘、描述此次执行的用途</i>
          <el-input v-model="modeInfo.remarks" placeholder="可无备注"></el-input>
        </el-form-item>
        <el-form-item label="病种:" prop="diseasename" >
          <i>请选择你要训练的疾病：</i>
          <div id="modleSelect">
            <el-radio-group id="disGroup" v-model="modeInfo.diseasename" ref="disGroup">
...
<!--       <el-upload
      ...
      <!-- 选择已有数据 -->
      <div>
        <h1 style="margin-top: 20px;margin-bottom: 20px;font-size: 20px">选择已有数据</h1>
      ...
      >
        ...
        <div style="padding: 14px">
          ...
          <div class="bottom clearfix">
            <el-button type="text" class="button" style="float: right" @click="toModeSelected(item.tablename)"
            >确认</el-button
            >
            <el-button
                type="text"
      ...
      <div>
        <el-button style="margin-top: 20px" @click="toBaseInfo" type="primary" >上一步</el-button>
        <el-button style="margin-top: 20px" @click="toModeSelect" type="primary" v-if="fileChoose">下一步</el-button>
      </div>
      </div>
    <!--  模型选择  -->
    <div class="mainPage" v-show=" showView === 'ModeSelect'"
         v-loading="loading" element-loading-text="拼命运算中" >
      <el-container>
        <el-tabs id="modelList" v-model="Algorithm" tab-position="left" >
          ...
          >
            ...
            <div class="mainPage .paramBox" v-loading="loading" element-loading-text="拼命运算中">
              <el-form :model="RF" :rules="RFRule" ref="RF" label-width="250px"  class="demo-ruleForm">
                ...
                <el-form-item label="目标特征:" prop="targetcolumn">
                  <el-radio-group v-model="RF.targetcolumn">
                    <el-radio
                        ...
                        :key="index" :label= "item.field_name" name="target"
                ...
                <el-form-item label="特征选择:" prop="fea">
                  ...
                  <el-checkbox-group v-model="RF.fea">
                    <el-checkbox v-for="(item,index) in remainedFeaData"
                                 :key="index" :label= "item.field_name" name="fea">
                      {{ item.field_name }}</el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
                <el-form-item label="参数估计量:" prop="n_estimators">
                    <el-input v-model.number="RF.n_estimators" placeholder="默认为10,数值越大，模型越稳定"></el-input>
                </el-form-item>
                <el-form-item label="叶子节点的最小样本数目:" prop="min_samples_leaf">
                  <el-input v-model.number="RF.min_samples_leaf" placeholder="叶子节点的最小样本数目，默认为1，数据量大的时候可以增大这个值;"></el-input>
                </el-form-item>
                <el-form-item label="限制子树继续划分的条件:" prop="min_samples_split">
                  <el-input v-model.number="RF.min_samples_split" placeholder="如果某节点的样本数目小于此值，则不会再继续划分，默认为2，样本量非常大的时候，应该增大这个值；"></el-input>
                </el-form-item>
                <el-form-item label="建立决策树时选择的最大特征数目:" prop="max_features">
                  <el-input v-model.number="RF.max_features" ></el-input>
                </el-form-item>
                <el-form-item label="构建树时是否使用bootstrap采样:" prop="min_samples_leaf">
...
export default {
  ...
  computed: {
    remainedFeaData() {
      return this.feaData.filter(item => item.field_name !== this.RF.targetcolumn);
    }
  },
  data(){
    return{
      ...
      modeInfo:{
        ...
        diseasename: '',
        remarks:"",
        Publisher: sessionStorage.getItem("user") ? sessionStorage.getItem("user") : '',
  ...
  methods: {
    ...
    getTableList(){
      getRequest(`/Model/tableName/${this.modeInfo.uid}`).then(
          res =>{
            if(res){
              // console.log(res)
    ...
    toModeSelect(){
      if(this.toThird){
        ...
        getRequest(`/Model/getFeature/${this.filename}`).then(
            res =>{
              if(res){
                this.feaData = res
    ...
    toModeSelected(name){
      ...
      getRequest(`/Model/getFeature/${this.chosenData}`).then(
          res =>{
            if(res){
              this.feaData = res
              console.log(this.feaData)
    ...
    saveMode(){
      postRequest('/Model/baseInfo', this.modeInfo).then(
          res => {
            if (res) {
              this.step = this.step + 1
